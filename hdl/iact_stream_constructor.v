...
module iact_stream_constructor #(
  ...
  generate
    ...
    reg [12-1:0] wr_addr_1_inc;
    reg [12-1:0] wr_addr_2_inc;
    reg [12-1:0] rd_addr_1;
    reg [12-1:0] rd_addr_2;
    reg [12-1:0] rd_addr_3;
    reg [12-1:0] rd_addr_4;
    reg [12-1:0] rd_addr_1_inc;
    reg [12-1:0] rd_addr_2_inc;
    reg [12-1:0] rd_addr_3_inc;
    reg [12-1:0] rd_addr_4_inc;
        ...
        case (fsm_enc_current_state)
          ...
          ENCODE: begin
            ...
            if (((ram_inc_counter[7:0] + 1) % WORDS_PER_CYCLE[7:0] == WORDS_PER_CYCLE[7:0] - 1)) begin
              ...
              if (fsm_enc_cycle >= (needed_iact_router_cycles_reg * wght_size_y * (iact_channels_i)) - 1) begin
                ...
                if (iact_channel_counter == needed_iact_channel_cycles_i - 1 | fully_connected_i) begin
                  ...
                  if (wght_counter == needed_wght_cycles_i - 1 | fully_connected_i) begin
                    ...
                    if (x_line_counter == x_lines_i - 1) begin
                      ...
                      if (iact_channels_i == 1) begin
                        rd_addr_4 <= (ram_rd_addr_reset_value+1)/2;
                      end else begin
    ...
    always @(posedge clk_i, negedge rst_ni) begin
      ...
      if (!rst_ni) begin
        ...
        x_times_c           <= iact_size_x_i * iact_channels_i;
        ...
        case (fsm_current_state)
          ...
          GET_PARAMETER: begin
            ...
            kernel_y_counter    <= 0;
            byte_var_pre_calc   <= 0;
            padding_x          <= wght_size_x / 2;
            padding_y          <= wght_size_y / 2;
            ...
            lines_in_words<= (iact_size_y_i+(padding_y*2));
            if (iact_channels_i == 1) begin
              lines_in_words <= ((iact_size_y_i+(padding_y*2)+1)/2);
            end
            if (enable_store) begin
          ...
          WRITE_TO_MEMORY: begin
            ...
            if (x_cycle == (iacts_in_one_trans * needed_iact_router_cycles_reg) - 1) begin
              x_cycle <= 0;
              y_cycle <= y_cycle + 1;
              x       <= x + (PE_X * CLUSTERS * stride_x_i);
              if (y_cycle == x_lines_i- 1) begin
                y_cycle <= 0;
                x       <= x - ((x_lines_i - 1) * (PE_X * CLUSTERS * stride_x_i));
                y       <= y + 1;
                if (iact_channels_i == 1) begin
                  y <= y + WORDS_PER_CYCLE;
                end
            ...
            for (r = 0; r < NUM_GLB_IACT; r=r+1) begin
              for (w = 0; w < WORDS_PER_CYCLE; w=w+1) begin
                if (fully_connected_i) begin
                  byte_var = w + (((channels * PE_Y * iact_channels_i) + (r * iact_channels_i)) ) + (byte_var_pre_calc/(2/WORDS_PER_CYCLE))* 2;
                  ram_var  = (byte_var  / 8);
                end else begin
                  ram_var = iact_channels_i == 1 ? w : 0;
                  ram_var = (((channels * x_times_y) + iact_channels_i * (((ram_var + y_reg) * iact_size_x_i) + x_reg[r])) / 8);
                  byte_var = (y_times_c * (iact_size_x_i%2)) + (x_times_c * ((iact_channels_i == 1 ? y_reg + w: y_reg)));
