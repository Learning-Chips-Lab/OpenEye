        ...
        CALCULATING: begin
          /*if (data_mode_reg) begin
            ...
            if (psum_spad_addr_a_r == psum_spad_addr_b_w) begin
              // Port A read conflicts with Port B write (cross-port forwarding)
              reuse_psum_spad_a <= 1;
              reused_data_a     <= adder_2_o_w; // Use adder 2 output
            end
            if (psum_spad_addr_b_r == psum_spad_addr_a_w) begin
              // Port B read conflicts with Port A write (cross-port forwarding)
              reuse_psum_spad_b <= 1;
