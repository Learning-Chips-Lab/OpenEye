...
// Module: PE_dense_tb
//
// Testbench for the Processing Element (PE) module with DENSE data.
// This testbench performs a complete 1D convolution operation with:
// - Input activations: [4, 5, 6, 7, 8]
// - Weights (filter):  [1, 2, 3, 4, 5]
// - Bias:              10
// - Expected output:   10 + (4*1 + 5*2 + 6*3 + 7*4 + 8*5) = 10 + 110 = 120
//
// The testbench demonstrates:
// - Data loading into SPAD memories using dense (non-sparse) format
// - Triggering MAC (Multiply-Accumulate) computation with dense data
// - Loading bias/partial sum values
// - Validating output results
//
// Unlike the sparse testbench, this uses dense data where all values are
// non-zero and the overhead encoding indicates no zeros are skipped.
//
...
module PE_dense_tb;
  ...
  // Test configuration - DENSE data test
  parameter NUM_IACT = 5;  // Number of input activations (all non-zero)
  parameter NUM_WGHT = 5;  // Number of weights (all non-zero)
  ...
  // Test data arrays - Dense convolution test
  ...
  initial begin
    if (CREATE_VCD) begin
      $dumpfile("PE_dense_tb.vcd");
      $dumpvars(0, PE_dense_tb);
    end
  end
  ...
  // Task: Load input activations into SPAD (DENSE format)
  // All values are non-zero, overhead encoding is 0 for all elements
  task load_input_activations;
    integer addr_idx, data_idx;
    begin
      $display("[%0t] Loading dense input activations...", $time);

      // Enable IACT interface
      iact_enable_i = 3'b001;  // Enable first channel

      // Send address array first
      // For dense data: addr_data[0] = NUM_IACT (all elements are present)
      @(posedge clk_i);
      iact_addr_data[0] = NUM_IACT;  // Number of non-zero elements (all 5)
      iact_addr_data[1] = NUM_IACT;  // Cumulative count
      iact_data_i = {48'h0, iact_addr_data[0]};

      @(posedge clk_i);
      iact_data_i = {48'h0, iact_addr_data[1]};

      // Send data array - DENSE format
      // Format: [overhead (4-bit) | data (8-bit)] = 12 bits per element
      // overhead = 0 for all dense elements (no zeros skipped)
      for (data_idx = 0; data_idx < NUM_IACT; data_idx = data_idx + 1) begin
        @(posedge clk_i);
        iact_data_array[data_idx] = {16'h0, 4'h0, test_iact[data_idx]};  // overhead=0
        iact_data_i = {48'h0, iact_data_array[data_idx]};
        $display("  [%0t] Loading iact[%0d] = %0d (overhead=0)", $time, data_idx, test_iact[data_idx]);
      end
      ...
      $display("[%0t] Dense input activations loaded (all %0d values)", $time, NUM_IACT);
    end
  endtask

  // Task: Load weights into SPAD (DENSE format)
  task load_weights;
    integer addr_idx, data_idx;
    reg [11:0] wght_with_overhead;
    integer num_packed_words;
    begin
      $display("[%0t] Loading dense weights...", $time);

      // Enable weight interface
      wght_enable_i = 1'b1;

      // Calculate number of packed words (2 weights per word for PARALLEL_MACS=2)
      num_packed_words = (NUM_WGHT + PARALLEL_MACS - 1) / PARALLEL_MACS;
      ...
      @(posedge clk_i);
      wght_addr_data[1] = num_packed_words;  // End marker
      wght_data_i = wght_addr_data[1];

      @(posedge clk_i);
      wght_addr_data[2] = num_packed_words;  // Repeat for single filter
      ...
      for (data_idx = 0; data_idx < NUM_WGHT; data_idx = data_idx + 2) begin
        @(posedge clk_i);
        if (data_idx + 1 < NUM_WGHT) begin
          // Pack two weights
          wght_data_array[data_idx/2] = {4'h0, test_wght[data_idx], 4'h0, test_wght[data_idx+1]};
          $display("  [%0t] Loading wght[%0d]=%0d, wght[%0d]=%0d (overhead=0 for both)",
                   $time, data_idx, test_wght[data_idx], data_idx+1, test_wght[data_idx+1]);
        end else begin
          // Last weight (odd number of weights)
          wght_data_array[data_idx/2] = {4'h0, test_wght[data_idx], 12'h0};
          $display("  [%0t] Loading wght[%0d]=%0d (overhead=0, padding)", $time, data_idx, test_wght[data_idx]);
        end
        wght_data_i = wght_data_array[data_idx/2];
      end
      ...
      $display("[%0t] Dense weights loaded (all %0d values)", $time, NUM_WGHT);
    end
  endtask
  ...
  task load_bias;
    begin
      ...
      $display("  [%0t] Bias = %0d", $time, test_bias[0]);
  ...
  task validate_results;
    ...
    begin
      ...
      if (result_ok) begin
        $display("*** DENSE CONVOLUTION TEST PASSED ***");
      end else begin
        $display("*** DENSE CONVOLUTION TEST FAILED ***");
      end
  ...
  // ============================================================================
  // Test Stimulus - Main Dense Convolution Test
  // ============================================================================
  ...
  initial begin
    ...
    // Setup test data for 1D convolution with DENSE (non-sparse) data
    // Input:   [4, 5, 6, 7, 8]
    // Filter:  [1, 2, 3, 4, 5]
    // Bias:    10
    // Result:  10 + (4*1 + 5*2 + 6*3 + 7*4 + 8*5) = 10 + (4 + 10 + 18 + 28 + 40) = 10 + 110 = 120
    test_iact[0] = 8'd4;
    test_iact[1] = 8'd5;
    test_iact[2] = 8'd6;
    test_iact[3] = 8'd7;
    ...
    // Print test banner
    $display("================================================================================");
    $display("PE Dense (Non-Sparse) Convolution Test");
    $display("================================================================================");
    $display("Clock Period: %0d ns", CLK_PERIOD);
    $display("");
    $display("Dense Convolution Configuration:");
    $display("  Input Activations: [%0d, %0d, %0d, %0d, %0d]",
             test_iact[0], test_iact[1], test_iact[2], test_iact[3], test_iact[4]);
    $display("  Filter Weights:    [%0d, %0d, %0d, %0d, %0d]",
             test_wght[0], test_wght[1], test_wght[2], test_wght[3], test_wght[4]);
    $display("  Bias:              %0d", test_bias[0]);
    $display("");
    $display("Expected Computation:");
    $display("  Output = Bias + (I[0]*W[0] + I[1]*W[1] + I[2]*W[2] + I[3]*W[3] + I[4]*W[4])");
    $display("         = %0d + (%0d*%0d + %0d*%0d + %0d*%0d + %0d*%0d + %0d*%0d)",
             test_bias[0],
             test_iact[0], test_wght[0],
             test_iact[1], test_wght[1],
             test_iact[2], test_wght[2],
             test_iact[3], test_wght[3],
             test_iact[4], test_wght[4]);
    ...
    $display("");
    $display("NOTE: This is DENSE data - all values are non-zero.");
    $display("      Overhead encoding is 0 for all elements (no zeros skipped).");
    $display("================================================================================");
    $display("");
    ...
    // ========================================================================
    // Execute Dense Convolution Test Sequence
    // ========================================================================

    $display("[%0t] ===== Starting Dense Convolution Test Sequence =====", $time);
    $display("");
    ...
    if (test_errors == 0) begin
      ...
      $display("*** DENSE TEST FAILED - %0d ERRORS ***", test_errors);
    end
