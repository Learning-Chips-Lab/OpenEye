...
@cocotb.test()
async def start_test_gemm_approach2(dut):
    """
    Approach 2 GEMM test.

    Drives gemm_mode_i=1 so the cluster hardware routes GLB bank j to
    every PE in row j automatically.  The test structure is otherwise
    identical to the standard conv test.
    """
    timeout_time = 40000
    timeout_unit = "ns"
    try:
        await with_timeout(_run_gemm_approach2(dut), timeout_time, timeout_unit)
    except SimTimeoutError:
        dut._log.error("Approach-2 GEMM test timed out!")
        raise


async def _run_gemm_approach2(dut):
    ...
    # Approach 2: assert GEMM mode — cluster overrides iact_select internally
    cocotb.start_soon(rtl_test_utils.set_input(ptp, dut.gemm_mode_i, 1))

    # Tie off systolic ports (SYSTOLIC_GEMM_EN=0 in this compilation)
...
if __name__ == "__main__":
    ...
    target_dir = os.path.join(test_dir, ".temp", "gemm_approach2_standalone")
    ...
    cocotb_test.simulator.run(
        ...
        extra_env={
            ...
            "SPARSE_IACT": "0",
            "SPARSE_WGHT": "0",
            "SEED":        str(int(sys.argv[4]) if len(sys.argv) > 4 else 42),
        },
    )
